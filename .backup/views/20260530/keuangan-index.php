...
<div x-data="keuanganPage()" x-init="init()">
    ...
    <div class="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-3">
        <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900">
            <p class="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Total Hutang</p>
            <p class="text-2xl font-bold text-rose-600" x-text="'Rp ' + summary.total_hutang.toLocaleString('id-ID')">
            </p>
            <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">ke supplier</p>
        </div>
        <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900">
            <p class="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Total Piutang
            </p>
            <p class="text-2xl font-bold text-emerald-600"
                x-text="'Rp ' + summary.total_piutang.toLocaleString('id-ID')"></p>
            <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">dari pembeli</p>
        </div>
        <div
            class="col-span-2 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm lg:col-span-1 dark:border-slate-700 dark:bg-slate-900">
            <p class="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Jatuh Tempo</p>
            <p class="text-2xl font-bold" :class="summary.overdue_count > 0 ? 'text-rose-600' : 'text-emerald-600'"
                x-text="summary.overdue_count + ' tagihan'"></p>
            <p class="mt-1 text-xs text-rose-600" x-show="summary.overdue_count > 0">⚠ Perlu perhatian!</p>
        </div>
    </div>
                ...
                <tbody>
                    ...
                    <template x-for="hp in filteredList" :key="hp.id">
                        <tr :id="'debt-' + hp.id" :data-highlight="'debt-' + hp.id">
                            ...
                            <td>
                                <span class="text-sm"
                                    :style="hp.hari_jatuh_tempo < 0 ? 'color: var(--color-danger); font-weight: bold;' : hp.hari_jatuh_tempo < 7 ? 'color: var(--color-warning)' : ''"
