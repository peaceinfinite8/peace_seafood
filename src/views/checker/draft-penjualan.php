...
<div x-data="checkerDraftPage()" x-init="init()">
    ...
    <div class="flex items-center gap-2 mb-5">
        <div class="flex items-center gap-1.5">
            <div class="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold"
                :style="step >= 1 ? 'background:var(--color-primary);color:#fff' : 'background:var(--bg-secondary);color:var(--text-secondary)'">
                1</div>
            <span class="text-xs font-medium"
                :style="step >= 1 ? 'color:var(--color-primary)' : 'color:var(--text-secondary)'">Pilih Produk</span>
        </div>
        <div class="flex-1 h-px" style="background:var(--border-color)"></div>
        <div class="flex items-center gap-1.5">
            <div class="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold"
                :style="step >= 2 ? 'background:var(--color-primary);color:#fff' : 'background:var(--bg-secondary);color:var(--text-secondary)'">
                2</div>
            <span class="text-xs font-medium"
                :style="step >= 2 ? 'color:var(--color-primary)' : 'color:var(--text-secondary)'">Isi Jumlah</span>
        </div>
        <div class="flex-1 h-px" style="background:var(--border-color)"></div>
        <div class="flex items-center gap-1.5">
            <div class="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold"
                :style="step >= 3 ? 'background:var(--color-primary);color:#fff' : 'background:var(--bg-secondary);color:var(--text-secondary)'">
                3</div>
            <span class="text-xs font-medium"
                :style="step >= 3 ? 'color:var(--color-primary)' : 'color:var(--text-secondary)'">Kirim</span>
        </div>
    </div>
    ...
    <div x-show="step === 1">
        <div class="card p-4 mb-4">
            <input type="text" x-model="search" class="form-input w-full" placeholder="🔍  Cari nama ikan..."
                @input="filterProduk()">
        </div>
        ...
        <div x-show="!loadingProduk && filteredProduk.length === 0" class="text-center py-10"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <template x-for="p in filteredProduk" :key="p.id">
                <button @click="selectProduk(p)" class="card p-4 text-left transition-all active:scale-95"
                    :class="selectedItems.find(i => i.id_produk == p.id) ? 'ring-2 ring-blue-500' : ''"
                    style="cursor:pointer">
                    <div class="flex items-start justify-between gap-2">
                        <div class="flex-1 min-w-0">
                            <p class="font-semibold text-sm leading-tight truncate" style="color:var(--text-primary)"
                                x-text="p.nama"></p>
                    ...
                    <div x-show="selectedItems.find(i => i.id_produk == p.id)"
                        ...
                        <span
                            x-text="'Dipilih: ' + (selectedItems.find(i => i.id_produk == p.id)?.qty || 0) + ' kg'"></span>
                    </div>
                </button>
            </template>
        </div>
    ...
    <div x-show="step === 2">
            ...
            <template x-for="(item, idx) in selectedItems" :key="item.id_produk">
                <div class="card p-4">
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <button @click="removeItem(idx)" class="ml-2 p-1.5 rounded-lg"
                    ...
                    <div class="mb-3">
                        <label class="text-xs font-semibold mb-1 block" style="color:var(--text-secondary)">JUMLAH
                            (kg)</label>
                        <div class="flex items-center gap-2">
                            ...
                            <input type="number" x-model="item.qty" @input="calcItem(idx)" min="0.01" step="0.5"
                                class="form-input text-center text-lg font-bold flex-1" style="height:44px"
                                placeholder="0">
                    ...
                    <div>
                        <label class="text-xs font-semibold mb-1 block" style="color:var(--text-secondary)">HARGA JUAL /
                            kg</label>
                        <input type="text" :value="item.harga_jual_fmt" @input="onHargaInput(idx, $event)"
                            inputmode="numeric" class="form-input text-right font-semibold" placeholder="0">
                    </div>

                    <!-- Subtotal -->
                    <div class="mt-3 flex justify-between items-center pt-3 border-t"
        ...
        <!-- Catatan pembeli -->
        <div class="card p-4 mb-4">
            <label class="text-xs font-semibold mb-1 block" style="color:var(--text-secondary)">NAMA PEMBELI
                (opsional)</label>
        ...
        <div class="card p-4 mb-4" style="background:rgba(37,99,235,0.04);border-color:rgba(37,99,235,0.2)">
            ...
            <p class="text-xs mt-1" style="color:var(--text-secondary)">*Harga final ditentukan kasir saat finalisasi
            </p>
        </div>
        ...
        <div class="flex gap-3">
            ...
            <button @click="step = 3" class="btn btn-primary flex-1 py-3 font-bold" :disabled="!isValid">
                Preview & Kirim
    ...
    <div x-show="step === 3">
        <div class="card p-4 mb-4">
            ...
            <template x-for="item in selectedItems" :key="item.id_produk">
                <div class="flex justify-between items-center py-2 border-b" style="border-color:var(--border-color)">
                    <div>
                        ...
                        <p class="text-xs" style="color:var(--text-secondary)"
                            x-text="parseFloat(item.qty).toLocaleString('id-ID') + ' kg × Rp ' + parseFloat(item.harga_jual||0).toLocaleString('id-ID')">
                        </p>
                    </div>
            ...
            <div class="mt-3 pt-3 border-t text-sm" style="border-color:var(--border-color)"
                x-show="namaPembeli || catatan">
                <p x-show="namaPembeli"><span style="color:var(--text-secondary)">Pembeli:</span> <strong
                        x-text="namaPembeli"></strong></p>
                <p x-show="catatan" class="mt-1"><span style="color:var(--text-secondary)">Catatan:</span> <span
                        x-text="catatan"></span></p>
            </div>
        </div>

        <!-- Info -->
        <div class="rounded-xl p-3 mb-5 flex gap-2"
            style="background:rgba(37,99,235,0.06);border:1px solid rgba(37,99,235,0.15)">
            <i data-lucide="info" class="w-4 h-4 flex-shrink-0 mt-0.5" style="color:var(--color-primary)"></i>
            <p class="text-xs" style="color:var(--text-secondary)">
                Draft ini akan dikirim ke <strong>Admin/Kasir</strong> untuk diproses. Anda akan mendapat konfirmasi
                setelah nota difinalisasi.
            </p>
        </div>
        ...
        <div class="flex gap-3">
            ...
            <button @click="kirimDraft()" class="btn btn-primary flex-1 py-3 font-bold text-base" :disabled="sending">
    ...
    <div x-show="step === 4" class="text-center py-10">
        ...
        <p class="text-sm mb-6" style="color:var(--text-secondary)">Admin/Kasir sudah mendapat notifikasi dan akan
            segera memproses.</p>
...
<?php $scripts = '<script src="/peace_seafood/inline-assets/js/checker/draft-penjualan.js"></script>'; ?>