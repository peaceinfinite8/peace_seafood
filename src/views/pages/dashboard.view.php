...
<div x-data="dashboardPage()" x-init="init()" class="flex flex-col gap-6">
    <div class="mb-2">
        <h2 class="text-xl font-bold text-(--text-primary)">
            Selamat datang, <span x-text="user.name || 'User'"></span> | <span
                class="text-sm font-semibold uppercase text-(--color-primary)"
    ...
    <div x-show="!loading" x-cloak>
        ...
        <div x-show="user.role === 'saas_owner'" class="space-y-6" x-cloak>
            ...
            <div class="grid grid-cols-2 gap-4 text-left lg:grid-cols-4">
                <div class="rounded-2xl border-(--border-color) bg-(--bg-light) p-4 shadow-sm">
                    <div class="mb-3 flex items-center justify-between gap-3">
                        <span class="text-xs font-bold uppercase tracking-wider text-(--text-secondary)">Total Gudang
                            Tenant</span>
                        <div class="flex h-9 w-9 items-center justify-center rounded-lg bg-blue-500/10 text-blue-500">
                            <i data-lucide="layout-grid" class="h-4 w-4"></i>
                        </div>
                    </div>
                    <div class="text-2xl font-bold text-(--text-primary)" x-text="stats.total_gudang || 0"></div>
                    <p class="mt-1 text-[10px] text-(--text-secondary)">Gudang aktif & suspended</p>
                </div>

                <div class="rounded-2xl border-(--border-color) bg-(--bg-light) p-4 shadow-sm">
                    <div class="mb-3 flex items-center justify-between gap-3">
                        <span class="text-xs font-bold uppercase tracking-wider text-(--text-secondary)">Tenant
                            Trial/Sewa Aktif</span>
                        <div
                            class="flex h-9 w-9 items-center justify-center rounded-lg bg-emerald-500/10 text-emerald-500">
                            <i data-lucide="shield-check" class="h-4 w-4"></i>
                        </div>
                    </div>
                    <div class="text-2xl font-bold text-emerald-500" x-text="stats.active_gudang || 0"></div>
                    <p class="mt-1 text-[10px] text-(--text-secondary)">Gudang sewa masih berlaku</p>
                </div>

                <div class="rounded-2xl border-(--border-color) bg-(--bg-light) p-4 shadow-sm">
                    <div class="mb-3 flex items-center justify-between gap-3">
                        <span class="text-xs font-bold uppercase tracking-wider text-(--text-secondary)">Tenant Expired
                            / Suspend</span>
                        <div class="flex h-9 w-9 items-center justify-center rounded-lg bg-red-500/10 text-red-500">
                            <i data-lucide="lock" class="h-4 w-4"></i>
                        </div>
                    </div>
                    <div class="text-2xl font-bold text-red-500" x-text="stats.expired_gudang || 0"></div>
                    <p class="mt-1 text-[10px] text-(--text-secondary)">Gudang sewa habis / suspend</p>
                </div>

                <div class="rounded-2xl border-(--border-color) bg-(--bg-light) p-4 shadow-sm">
                    <div class="mb-3 flex items-center justify-between gap-3">
                        <span class="text-xs font-bold uppercase tracking-wider text-(--text-secondary)">Belum
                            Onboarding</span>
                        <div class="flex h-9 w-9 items-center justify-center rounded-lg bg-amber-500/10 text-amber-500">
                            <i data-lucide="clock" class="h-4 w-4"></i>
                        </div>
                    </div>
                    <div class="text-2xl font-bold text-amber-500" x-text="stats.pending_onboarding || 0"></div>
                    <p class="mt-1 text-[10px] text-(--text-secondary)">Gudang terdaftar, trial belum aktif</p>
                </div>
            </div>

            <!-- Volume Transaksi Platform row -->
            <div class="flex items-center gap-4 rounded-2xl border-(--border-color) bg-(--bg-light) p-4 shadow-sm">
                <div
                    class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-cyan-500/10 text-cyan-500">
                    <i data-lucide="wallet" class="h-5 w-5"></i>
                </div>
                <div class="flex-1">
                    <p class="text-xs font-bold uppercase tracking-wider text-(--text-secondary)">Volume Transaksi
                        Seluruh Platform</p>
                    <p class="mt-0.5 text-xl font-bold text-cyan-500" x-text="formatRupiah(stats.total_sales_all || 0)">
                    </p>
                    <p class="mt-0.5 text-[10px] text-(--text-secondary)"><span
                            x-text="stats.total_sales_count || 0"></span> nota penjualan lunas dari semua gudang</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
                ...
                <div
                    class="space-y-4 rounded-2xl border-(--border-color) bg-(--bg-light) p-5 text-left shadow-sm lg:col-span-2">
                    <div class="flex items-center gap-2 border-b border-(--border-color) pb-2">
                        <i data-lucide="user-plus" class="h-4 w-4 text-blue-500"></i>
                        <h4 class="text-sm font-bold text-(--text-primary)">Pra-Persetujuan (Pre-Approve) Akun Bos Baru
                        </h4>
                    </div>

                    <form @submit.prevent="runPreApprove()" class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div>
                            <label
                                class="mb-1.5 block text-2xs font-bold uppercase tracking-wider text-(--text-secondary)">Nama
                                Lengkap Calon Bos *</label>
                            <input type="text" x-model="preApproveForm.name"
                                class="w-full rounded-lg border border-(--border-color) bg-(--bg-light) px-3 py-2 text-xs text-(--text-primary) outline-none transition focus:border-(--color-primary) focus:ring-2 focus:ring-(--color-primary-light)"
                                placeholder="Contoh: Bos Ronald" required>
                        </div>
                        <div>
                            <label
                                class="mb-1.5 block text-2xs font-bold uppercase tracking-wider text-(--text-secondary)">Email
                                Gmail Calon Bos *</label>
                            <input type="email" x-model="preApproveForm.email"
                                class="w-full rounded-lg border border-(--border-color) bg-(--bg-light) px-3 py-2 text-xs text-(--text-primary) outline-none transition focus:border-(--color-primary) focus:ring-2 focus:ring-(--color-primary-light)"
                                placeholder="Contoh: ronald@gmail.com" required>
                        </div>
                        <div class="sm:col-span-2 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                            <div class="flex-1">
                                <label
                                    class="mb-1.5 block text-2xs font-bold uppercase tracking-wider text-(--text-secondary)">Durasi
                            ...
                            <button type="submit"
                                class="inline-flex h-[38px] items-center justify-center gap-1.5 rounded-lg bg-(--color-primary) px-5 py-2 text-xs font-semibold text-white shadow-lg shadow-blue-500/10 transition hover:opacity-95"
                ...
                <div class="space-y-4 rounded-2xl border-(--border-color) bg-(--bg-light) p-5 text-left shadow-sm">
                    <div class="flex items-center gap-2 border-b border-(--border-color) pb-2">
                        <i data-lucide="message-circle" class="h-4 w-4 text-emerald-500"></i>
                        <h4 class="text-sm font-bold text-(--text-primary)">WhatsApp Bantuan Platform</h4>
                    </div>

                    <div class="space-y-3">
                        <label class="block text-2xs leading-normal text-(--text-secondary)">Nomor WhatsApp Developer
                            (Gunakan format 628xxx tanpa tanda + atau 0 di depan)</label>
                        <div class="flex gap-2">
                            <input type="text" x-model="devWhatsappNumber"
                                class="w-full flex-1 rounded-lg border border-(--border-color) bg-(--bg-light) px-3 py-2 text-xs text-(--text-primary) outline-none transition focus:border-(--color-primary) focus:ring-2 focus:ring-(--color-primary-light)"
                                placeholder="Contoh: 628123456789">
                            <button type="button" @click="saveDevWhatsapp()"
                                class="inline-flex items-center justify-center rounded-lg bg-(--color-primary) p-2 text-white shadow-sm transition hover:opacity-95"
                                :disabled="savingWa">
            ...
            <div class="overflow-hidden rounded-2xl border-(--border-color) bg-(--bg-light) shadow-sm">
                <div class="border-b border-(--border-color) bg-(--bg-secondary)/40 p-4 text-left">
                    <h4 class="flex items-center gap-1.5 text-sm font-bold text-(--text-primary)">
                        <i data-lucide="layout-grid" class="h-4.5 w-4.5 text-blue-500"></i>
                    ...
                    <section class="dashboard-section card">
                        ...
                        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                            ...
                            <?php if (in_array(getWebUserRole(), ['admin', 'super_admin'], true)): ?>
                                <a href="${window.APP_BASE_URL}/penjualan?filter=draft" class="kpi-card block no-underline"
                    ...
                    <div class="flex flex-col gap-6">
                        <div class="card" x-show="['bos','admin','super_admin'].includes(user.role)">
                            <div class="flex items-center justify-between p-5 border-b muted-border"
                                ...
                                <a href="${window.APP_BASE_URL}/penjualan" class="text-xs"
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            <div class="card">
                                <div class="flex items-center justify-between p-5 border-b muted-border"
                                    ...
                                    <a href="${window.APP_BASE_URL}/stok" class="text-xs"
...
function dashboardPage() {
    ...
    return {
        ...
        async loadDashboard() {
            try {
                ...
                if (this.user.role === 'saas_owner') {
                    const res = await axios.get(`${window.API_BASE_URL}/dashboard`, { headers });
                    if (res.data?.success) {
                ...
                const [dashRes, notaRes, stokRes] = await Promise.allSettled([
                    axios.get(`${window.API_BASE_URL}/dashboard`, { headers }),
                    axios.get(`${window.API_BASE_URL}/penjualan?per_page=5`, { headers }),
                ...
                if (this.user.role === 'bos' || this.user.role === 'admin' || this.user.role === 'checker') {
                    try {
                        const settingsRes = await axios.get(`${window.API_BASE_URL}/settings`, { headers });
        ...
        async updateGudangSubscription(g, newDate) {
            try {
                const token = localStorage.getItem('token');
                await axios.put(`${window.API_BASE_URL}/settings/gudang/${g.id}`, {
        ...
        async impersonateUser(idBos) {
            ...
            try {
                ...
                const token = localStorage.getItem('token');
                const res = await axios.post(`${window.API_BASE_URL}/auth/impersonate`, { user_id: idBos }, {
                    headers: { Authorization: 'Bearer ' + token }
                });
                if (res.data?.success) {
                    // Save new session
