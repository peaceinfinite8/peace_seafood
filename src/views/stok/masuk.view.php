...
<div x-data="stokMasukPage()" x-init="init()">
    ...
    <div class="flex items-center gap-4 mb-6">
        <a href="${window.APP_BASE_URL}/stok"
            class="inline-flex items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-100 dark:border-slate-600 dark:bg-slate-700 dark:text-white dark:hover:bg-slate-600 p-2">
    ...
    <div class="rounded-lg border border-slate-300 bg-white dark:border-slate-600 dark:bg-slate-800 p-6 max-w-2xl">
        <form @submit.prevent="submit()" novalidate>

            <!-- Penanggung Jawab -->
            <div class="space-y-1.5">
                <label class="block text-sm font-medium text-slate-700 dark:text-slate-300">Penanggung Jawab</label>
                <input type="text"
            ...
            <!-- Supplier -->
            <div class="space-y-1.5 relative animate-fade-in" x-data="{ open: false }" @click.outside="open = false">
                <label class="block text-sm font-medium text-slate-700 dark:text-slate-300">Supplier <span
                        class="text-red-500">*</span></label>
                <div class="relative">
                    <input type="text" x-model="supplierQuery" @focus="open = true" @input="onSupplierInput()"
                        class="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 dark:border-slate-600 dark:bg-slate-800 dark:text-white pr-10"
                        placeholder="Ketik nama supplier baru atau pilih dari daftar..." required>
            ...
            <!-- Produk -->
            <div class="space-y-1.5">
                <label class="block text-sm font-medium text-slate-700 dark:text-slate-300">Produk <span
                        class="text-red-500">*</span></label>
                <select x-model="form.id_produk"
                    class="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 dark:border-slate-600 dark:bg-slate-800 dark:text-white"
                    required @change="setProdukInfo()">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="space-y-1.5">
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300">Qty / Jumlah Stok Masuk
                        <span class="text-red-500">*</span></label>
                    <div class="grid grid-cols-3 gap-2">
                        <div>
                            <span class="text-[10px] mb-1 block text-center"
                                style="color: var(--text-secondary)">Ton</span>
                            <input type="number" x-model="form.qty_ton"
                                class="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 dark:border-slate-600 dark:bg-slate-800 dark:text-white text-center"
                                placeholder="0" min="0" step="1" @input="updateQtyFromSplit()">
                        </div>
                        <div>
                            <span class="text-[10px] mb-1 block text-center"
                                style="color: var(--text-secondary)">Kintal</span>
                            <input type="number" x-model="form.qty_kuintal"
                                class="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 dark:border-slate-600 dark:bg-slate-800 dark:text-white text-center"
                                placeholder="0" min="0" step="1" @input="updateQtyFromSplit()">
                        </div>
                        <div>
                            ...
                            <input type="number" x-model="form.qty_kg"
                                class="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 dark:border-slate-600 dark:bg-slate-800 dark:text-white text-center"
                                placeholder="0" min="0" step="0.01" @input="updateQtyFromSplit()">
                        </div>
                    </div>
                ...
                <div class="space-y-1.5">
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300">Harga Beli / kg <span
                            class="text-red-500">*</span></label>
                    <div style="height: 19px" class="hidden md:block"></div>
                    <input type="number" x-model="form.harga_beli"
                        class="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 dark:border-slate-600 dark:bg-slate-800 dark:text-white"
                        placeholder="0" min="0" step="100" @input="calcTotal()" required>
            ...
            <div class="flex gap-3">
                ...
                <a href="${window.APP_BASE_URL}/stok"
                    class="inline-flex items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-100 dark:border-slate-600 dark:bg-slate-700 dark:text-white dark:hover:bg-slate-600">Batal</a>
            </div>
        </form>
    </div>
</div>
...
function stokMasukPage() {
    return {
        ...
        async submit() {
            ...
            try {
                const token = localStorage.getItem('token');
                await axios.post(`${window.API_BASE_URL}/stok/masuk`, this.form, { headers: { Authorization: 'Bearer ' + token } });
                iziToast.success({ title: 'Berhasil', message: 'Stok masuk tersimpan! Menunggu timbangan.', position: 'topRight' });
                setTimeout(() => window.location.href = `${window.APP_BASE_URL}/stok`, 1000);
            } catch(e) {
