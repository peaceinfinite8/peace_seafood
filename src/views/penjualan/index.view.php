    ...
    <div class="no-print">
        ...
        <div class="card p-4 mb-6">
            <div class="flex flex-wrap gap-3">
                <input type="text" x-model="search" placeholder="Cari no nota / pembeli..." class="form-input flex-1 min-w-48">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6" x-show="!loading" x-cloak>
            ...
            <div class="stat-card">
                ...
                <p class="text-lg font-bold" style="color: var(--color-success)"
                    x-text="'Rp ' + notaList.filter(n=>n.status==='final').reduce((s,n)=>s+parseFloat(n.total),0).toLocaleString('id-ID')"></p>
            </div>
            <div class="stat-card">
                    ...
                    <tbody>
                        ...
                        <template x-for="nota in filteredNota" :key="nota.id">
                            <tr>
                                ...
                                <td><span class="font-semibold" style="color:var(--color-success)" x-text="'Rp ' + parseFloat(nota.total).toLocaleString('id-ID')"></span></td>
                                <td>
                                    <span class="badge"
                                ...
                                <td>
                                    <div class="flex gap-2">
                                        ...
                                        <button x-show="nota.status === 'final' && (nota.pembayaran || nota.jenis_pembayaran) === 'hutang' && nota.status_pembayaran !== 'lunas' && ['admin','super_admin'].includes(user.role)"
                                            ...
                                            class="btn btn-success p-1.5" title="Bayar Cicilan">
                                            <i data-lucide="credit-card" class="w-3.5 h-3.5"></i>
                                        </button>
                                        <button x-show="nota.status !== 'cancelled' && ['admin','super_admin'].includes(user.role)"
                                            @click="cancelNota(nota.id)"
                                            class="btn btn-danger p-1.5" title="Cancel">
                ...
                <template x-if="detail">
                    <div>
                        <div class="grid grid-cols-2 gap-4 mb-4 text-sm">
                            <div><span style="color:var(--text-secondary)">No Nota:</span> <strong x-text="detail.no_nota || '-' "></strong></div>
                            <div><span style="color:var(--text-secondary)">Tanggal:</span> <strong x-text="formatDate(detail.tanggal_nota)"></strong></div>
                            <div><span style="color:var(--text-secondary)">Pembeli:</span> <strong x-text="detail.nama_pembeli || 'Umum'"></strong></div>
                            ...
                            <div>
                                ...
                                <span class="badge ml-2" :class="detail.status_pembayaran === 'lunas' ? 'badge-success' : 'badge-warning'" x-text="detail.status_pembayaran ? detail.status_pembayaran.toUpperCase() : '-' "></span>
                                <span class="text-xs ml-2" style="color:var(--text-secondary)" x-show="detail.status_pembayaran && detail.status_pembayaran !== 'lunas'" x-text="'Sisa: Rp ' + parseFloat(detail.sisa_tagihan || 0).toLocaleString('id-ID')"></span>
                            </div>
                            <div x-show="detail.pembayaran === 'transfer'"><span style="color:var(--text-secondary)">Bank Tujuan:</span> <strong x-text="detail.nama_bank ? detail.nama_bank + ' - ' + detail.account_name : '-' "></strong></div>
                        </div>
                        ...
                        <div class="text-right space-y-1 text-sm">
                            ...
                            <div x-show="parseFloat(detail.diskon_nominal||0) > 0">Diskon: <strong class="text-red-500" x-text="'- ' + formatRupiah(detail.diskon_nominal)"></strong></div>
                            <div x-show="parseFloat(detail.pajak||0) > 0">Pajak: <strong x-text="'+ ' + formatRupiah(detail.pajak)"></strong></div>
                        ...
                        <template x-if="detail.pembayaran === 'hutang' && detail.payments && detail.payments.length > 0">
                            <div class="mt-6 pt-4 border-t" style="border-color: var(--border-color)">
                                <h4 class="font-bold text-sm mb-2" style="color: var(--text-primary)">Riwayat Pembayaran Cicilan</h4>
                                <div class="overflow-x-auto">
                                    <table class="table text-xs">
                                        ...
                                        <tbody>
                                            <template x-for="p in detail.payments" :key="p.id">
                                                <tr>
                                                    <td class="whitespace-nowrap" x-text="formatDate(p.created_at)"></td>
                                                    <td class="font-semibold text-green-600 dark:text-green-400 whitespace-nowrap" x-text="formatRupiah(p.nominal_bayar)"></td>
                        ...
                        <div class="mt-6 pt-4 border-t flex flex-wrap items-center justify-between gap-4" style="border-color: var(--border-color)">
                            <div class="flex items-center gap-2">
                                <label class="text-xs font-semibold" style="color: var(--text-secondary)">Pilih Kertas:</label>
        ...
        <div class="modal-overlay" x-show="showBayarModal" @click.self="showBayarModal = false" x-cloak>
            <div class="modal-box">
                ...
                <div x-show="selectedNota" class="mb-4 space-y-1.5 p-3 rounded-lg bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700">
                    <p class="text-sm"><span style="color:var(--text-secondary)">No Nota:</span> <strong x-text="selectedNota?.no_nota"></strong></p>
                    <p class="text-sm"><span style="color:var(--text-secondary)">Pembeli:</span> <strong x-text="selectedNota?.nama_pembeli || 'Umum'"></strong></p>
                    <p class="text-sm"><span style="color:var(--text-secondary)">Sisa Tagihan:</span> <strong class="text-red-500" x-text="'Rp ' + parseFloat(selectedNota?.sisa_tagihan||0).toLocaleString('id-ID')"></strong></p>
                </div>
                <div class="form-group mb-4">
                    <label class="form-label block text-sm font-semibold mb-1">Nominal Bayar <span class="text-red-500">*</span></label>
                    <input type="text"
        ...
        <template x-if="detail">
            <div>
                ...
                <div x-show="printLayout === 'a4' || printLayout === 'a5'">
                    <div style="display: flex; justify-content: space-between; align-items: flex-start; border-bottom: 2px solid #333; padding-bottom: 10px;">
                        <div>
                            ...
                            <p style="margin: 2px 0 0 0; font-size: 11px; color: #555;">Supplier Ikan Segar & Berkualitas</p>
                            <p style="margin: 2px 0 0 0; font-size: 11px; color: #555;" x-text="'Gudang: ' + (detail.nama_gudang || '-')"></p>
                        </div>
                        <div style="text-align: right;">
                            <h2 style="font-size: 16px; font-weight: bold; margin: 0; color: #333;">NOTA PENJUALAN</h2>
                            <p style="margin: 3px 0 0 0; font-family: monospace; font-size: 12px;" x-text="detail.no_nota"></p>
                        </div>
                    </div>

                    <div style="display: grid; grid-template-columns: 1fr 1fr; margin-top: 15px; font-size: 12px; gap: 20px;">
                        <div>
                            <table style="width: 100%; border: none; margin: 0;">
                                <tr style="border: none;">
                                    <td style="width: 80px; padding: 2px 0; border: none; font-weight: bold;">Tanggal</td>
                                    <td style="padding: 2px 0; border: none;" x-text="': ' + formatDate(detail.tanggal_nota)"></td>
                                </tr>
                                <tr style="border: none;">
                                    <td style="padding: 2px 0; border: none; font-weight: bold;">Kasir</td>
                                    <td style="padding: 2px 0; border: none;" x-text="': ' + (detail.nama_user || '-')"></td>
                                </tr>
                                <tr style="border: none;">
                                    <td style="padding: 2px 0; border: none; font-weight: bold;">Pembayaran</td>
                                    <td style="padding: 2px 0; border: none;" x-text="': ' + (detail.pembayaran || '-').toUpperCase() + (detail.status_pembayaran === 'lunas' ? ' (LUNAS)' : ' (BELUM LUNAS)')"></td>
                                </tr>
                            </table>
                        </div>
                        <div style="text-align: right;">
                            <p style="margin: 0; font-weight: bold;">Kepada Yth.</p>
                            <p style="margin: 2px 0 0 0; font-size: 14px; font-weight: bold;" x-text="detail.nama_pembeli || 'Umum'"></p>
                        </div>
                    </div>
                    ...
                    <table style="width: 100%; border-collapse: collapse; margin-top: 15px;">
                        <thead>
                            <tr style="border-bottom: 2px solid #000; border-top: 1px solid #ddd;">
                                ...
                                <th style="padding: 8px 4px; text-align: right; font-weight: bold; width: 100px;">Qty</th>
                                <th style="padding: 8px 4px; text-align: right; font-weight: bold; width: 120px;">Harga Satuan</th>
                                <th style="padding: 8px 4px; text-align: right; font-weight: bold; width: 120px;">Subtotal</th>
                            </tr>
                        </thead>
                        <tbody>
                            <template x-for="item in detail.items" :key="item.id">
                                <tr style="border-bottom: 1px solid #eee;">
                                    ...
                                    <td style="padding: 8px 4px; text-align: right;" x-text="formatWeight(item.qty, item.satuan)"></td>
                                    <td style="padding: 8px 4px; text-align: right;" x-text="formatRupiah(item.harga_jual)"></td>
                                    <td style="padding: 8px 4px; text-align: right;" x-text="formatRupiah(item.subtotal)"></td>
                                </tr>
                            </template>
                        </tbody>
                    </table>
                    ...
                    <div style="display: flex; justify-content: space-between; margin-top: 15px; font-size: 12px;">
                        <div style="width: 50%;">
                            <p style="margin: 0 0 5px 0; font-style: italic;" x-show="detail.catatan" x-text="'Catatan: ' + detail.catatan"></p>
                            <div x-show="detail.pembayaran === 'transfer' && detail.nama_bank" style="background-color: #f9f9f9; padding: 8px; border-radius: 4px; border: 1px solid #eee; display: inline-block;">
                                <p style="margin: 0; font-weight: bold; font-size: 11px;">Info Transfer Bank:</p>
                                <p style="margin: 2px 0 0 0; font-size: 11px;" x-text="detail.nama_bank + ' - ' + detail.account_name"></p>
                                <p style="margin: 2px 0 0 0; font-size: 11px;" x-text="'No. Rek: ' + detail.account_number"></p>
                            </div>

                            <!-- Riwayat Cicilan (A4/A5) -->
                            <div x-show="detail.pembayaran === 'hutang' && detail.payments && detail.payments.length > 0" style="margin-top: 15px;">
                                <p style="margin: 0 0 5px 0; font-weight: bold; font-size: 11px;">Riwayat Cicilan:</p>
                                <table style="width: 100%; border-collapse: collapse; font-size: 10px; text-align: left; border: 1px solid #ddd;">
                                    <thead>
                                        <tr style="background-color: #f9f9f9; border-bottom: 1px solid #ddd;">
                                            <th style="padding: 4px; border: 1px solid #ddd;">Tanggal</th>
                                            <th style="padding: 4px; border: 1px solid #ddd; text-align: right;">Nominal</th>
                                    ...
                                    <tbody>
                                        <template x-for="p in detail.payments" :key="p.id">
                                            <tr style="border-bottom: 1px solid #eee;">
                                                <td style="padding: 4px; border: 1px solid #ddd;" x-text="formatDate(p.created_at)"></td>
                                                <td style="padding: 4px; border: 1px solid #ddd; text-align: right;" x-text="formatRupiah(p.nominal_bayar)"></td>
                                                <td style="padding: 4px; border: 1px solid #ddd;" x-text="p.keterangan || '-'"></td>
                                            </tr>
                                        </template>
                                    </tbody>
                                </table>
                        ...
                        <div style="width: 40%;">
                            <table style="width: 100%; border: none; margin: 0;">
                                <tr style="border: none;">
                                    <td style="padding: 4px 0; text-align: right; border: none; color: #555;">Subtotal:</td>
                                    <td style="padding: 4px 0; text-align: right; border: none; font-weight: bold; width: 120px;" x-text="formatRupiah(detail.subtotal)"></td>
                                </tr>
                                <tr style="border: none;" x-show="parseFloat(detail.diskon_nominal || 0) > 0">
                                    <td style="padding: 4px 0; text-align: right; border: none; color: #555;">Potongan:</td>
                                    <td style="padding: 4px 0; text-align: right; border: none; font-weight: bold; color: #d32f2f;" x-text="'- ' + formatRupiah(detail.diskon_nominal)"></td>
                                </tr>
                                <tr style="border: none;" x-show="parseFloat(detail.pajak || 0) > 0">
                                    <td style="padding: 4px 0; text-align: right; border: none; color: #555;">Pajak:</td>
                                    <td style="padding: 4px 0; text-align: right; border: none; font-weight: bold;" x-text="'+ ' + formatRupiah(detail.pajak)"></td>
                                </tr>
                                <tr style="border-top: 1.5px solid #333;">
                                    <td style="padding: 6px 0; text-align: right; border: none; font-weight: bold; font-size: 14px;">TOTAL:</td>
                                    <td style="padding: 6px 0; text-align: right; border: none; font-weight: bold; font-size: 14px; color: #111;" x-text="formatRupiah(detail.total)"></td>
                                </tr>
                            </table>
                        </div>
                    </div>
                    ...
                    <div style="display: flex; justify-content: space-between; margin-top: 50px; font-size: 12px; text-align: center;">
                        ...
                        <div style="width: 150px;">
                            ...
                            <div style="border-top: 1px solid #333; padding-top: 5px;" x-text="detail.nama_user || 'Kasir'"></div>
                        </div>
                    </div>
                </div>
                ...
                <div x-show="printLayout === 'thermal' || printLayout === 'thermal58'" style="font-family: 'Courier New', Courier, monospace;">
                    ...
                    <div style="font-size: 9px; line-height: 1.3; margin-bottom: 6px; border-bottom: 1px dashed #000; padding-bottom: 6px;">
                        <table style="width: 100%; border: none; margin: 0;">
                            <tr style="border: none;">
                                <td style="padding: 1px 0; border: none;">Tgl</td>
                                <td style="padding: 1px 0; border: none; text-align: right;" x-text="formatDate(detail.tanggal_nota)"></td>
                            </tr>
                            <tr style="border: none;">
                                <td style="padding: 1px 0; border: none;">Kasir</td>
                                <td style="padding: 1px 0; border: none; text-align: right;" x-text="detail.nama_user || '-'"></td>
                            </tr>
                            <tr style="border: none;">
                                <td style="padding: 1px 0; border: none;">Cust</td>
                                <td style="padding: 1px 0; border: none; text-align: right; font-weight: bold;" x-text="detail.nama_pembeli || 'Umum'"></td>
                            </tr>
                            <tr style="border: none;">
                                <td style="padding: 1px 0; border: none;">Bayar</td>
                                <td style="padding: 1px 0; border: none; text-align: right;" x-text="(detail.pembayaran || '-').toUpperCase()"></td>
                            </tr>
                        </table>
                    </div>
                    ...
                    <table style="width: 100%; border-collapse: collapse; font-size: 9px; margin-bottom: 6px;">
                        <tbody>
                            <template x-for="item in detail.items" :key="item.id">
                                <tr>
                                    <td colspan="3" style="padding: 3px 0 1px 0; font-weight: bold;" x-text="item.nama_produk"></td>
                                </tr>
                                <tr style="border-bottom: 1px solid #eee;">
                                    <td style="padding: 1px 0 3px 0;" x-text="formatWeight(item.qty, item.satuan)"></td>
                                    <td style="padding: 1px 0 3px 0; text-align: right;" x-text="formatRupiah(item.harga_jual)"></td>
                                    <td style="padding: 1px 0 3px 0; text-align: right; font-weight: bold;" x-text="formatRupiah(item.subtotal)"></td>
                                </tr>
                            </template>
                        </tbody>
                    </table>
                    ...
                    <div style="font-size: 9px; border-top: 1px dashed #000; padding-top: 4px; margin-bottom: 15px;">
                        <table style="width: 100%; border: none; margin: 0;">
                            <tr style="border: none;">
                                ...
                                <td style="padding: 1px 0; border: none; text-align: right;" x-text="formatRupiah(detail.subtotal)"></td>
                            </tr>
                            <tr style="border: none;" x-show="parseFloat(detail.diskon_nominal || 0) > 0">
                                <td style="padding: 1px 0; border: none; color: #d32f2f;">Diskon</td>
                                <td style="padding: 1px 0; border: none; text-align: right; color: #d32f2f;" x-text="'-' + formatRupiah(detail.diskon_nominal)"></td>
                            </tr>
                            <tr style="border: none;" x-show="parseFloat(detail.pajak || 0) > 0">
                                <td style="padding: 1px 0; border: none;">Pajak</td>
                                <td style="padding: 1px 0; border: none; text-align: right;" x-text="'+' + formatRupiah(detail.pajak)"></td>
                            </tr>
                            <tr style="border-top: 1px dashed #000;">
                                <td style="padding: 3px 0; border: none; font-weight: bold; font-size: 11px;">TOTAL</td>
                                <td style="padding: 3px 0; border: none; text-align: right; font-weight: bold; font-size: 11px;" x-text="formatRupiah(detail.total)"></td>
                            </tr>
                        </table>
                    </div>

                    <!-- Riwayat Cicilan (Thermal 80mm) -->
                    <div x-show="detail.pembayaran === 'hutang' && detail.payments && detail.payments.length > 0" style="font-size: 9px; border-top: 1px dashed #000; padding-top: 4px; margin-bottom: 10px;">
                            ...
                            <tbody>
                                <template x-for="p in detail.payments" :key="p.id">
                                    <tr style="border-bottom: 1px dotted #ccc;">
                                        ...
                                        <td style="padding: 2px 0; text-align: right;" x-text="formatRupiah(p.nominal_bayar)"></td>
                                    </tr>
                                    <template x-if="p.keterangan">
                                        <tr>
                                            <td colspan="2" style="padding: 0 0 4px 0; font-style: italic; color: #555; font-size: 8px;" x-text="'* ' + p.keterangan"></td>
                                        </tr>
                                    </template>
                                </template>
                            </tbody>
                    ...
                    <div style="text-align: center; font-size: 8px; margin-top: 10px; border-top: 1px dashed #000; padding-top: 6px;">
