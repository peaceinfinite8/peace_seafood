...
<div x-data="produkPage()" x-init="init()">
    ...
    <div class="card p-4 mb-4"><input type="text" x-model="search" placeholder="Cari produk..." class="form-input max-w-sm"></div>
    <div class="card">
                ...
                <tbody>
                    ...
                    <template x-for="p in filtered" :key="p.id">
                        <tr :id="'produk-' + p.id" :data-highlight="'produk-' + p.id">
                            ...
                            <td class="text-sm" x-text="'Rp ' + parseFloat(p.harga_beli||0).toLocaleString('id-ID')"></td>
                            ...
                            <td>
                                <div class="flex gap-2">
                                    <button @click="openEdit(p)" class="btn btn-secondary p-1.5" x-show="['super_admin','admin'].includes(user.role)"><i data-lucide="pencil" class="w-3.5 h-3.5"></i></button>
                                </div>
                            </td>
                        </tr>
                    </template>
    ...
    <div class="modal-overlay" x-show="showModal" @click.self="showModal = false" x-cloak>
        <div class="modal-box">
            ...
            <form @submit.prevent="save()">
                <div class="form-group"><label class="form-label">Nama Produk *</label><input type="text" x-model="form.nama" class="form-input" required></div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="form-group"><label class="form-label">Harga Beli</label><input type="text" inputmode="numeric" :value="form.harga_beli" @input="handleRupiahInput('harga_beli', $event)" class="form-input" placeholder="0"></div>
                    <div class="form-group"><label class="form-label">Harga Jual</label><input type="text" inputmode="numeric" :value="form.harga_jual" @input="handleRupiahInput('harga_jual', $event)" class="form-input" placeholder="0"></div>
                    <div class="form-group"><label class="form-label">Stok Minimum</label><input type="number" x-model="form.stok_minimum" class="form-input" min="0" step="0.1"></div>
