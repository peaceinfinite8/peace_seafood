...
<div x-data="migrationPage()" x-init="init()">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        ...
        <div class="flex flex-wrap items-center gap-2">
            <button type="button" @click="downloadTemplate('penjualan')" class="btn btn-outline flex items-center gap-2"
                style="border: 1px solid var(--border-color); color: var(--text-primary)">
                <i data-lucide="download" class="w-4 h-4 text-emerald-500"></i>
                Template Penjualan (CSV)
            </button>
            <button type="button" @click="downloadTemplate('stok')" class="btn btn-outline flex items-center gap-2"
                style="border: 1px solid var(--border-color); color: var(--text-primary)">
    ...
    <div
        class="card p-5 mb-6 bg-gradient-to-r from-blue-50/50 to-indigo-50/50 dark:from-slate-800/20 dark:to-indigo-950/10 border-l-4 border-blue-500 shadow-sm">
        <div class="flex gap-4">
            ...
            <div>
                <h4 class="font-bold text-sm" style="color: var(--text-primary)">Pusat Sinkronisasi Riwayat Transaksi
                </h4>
    ...
    <div class="keu-tab-group mb-6">
        <button @click="switchTab('excel')" class="keu-tab"
        ...
        <button @click="switchTab('ocr')" class="keu-tab"
    ...
    <div x-show="tab === 'excel'" class="space-y-6">
        ...
        <div x-show="!previewMode"
            class="card p-8 text-center border-2 border-dashed border-gray-300 dark:border-slate-700 hover:border-blue-500 transition-colors cursor-pointer relative"
        ...
        <div x-show="previewMode && !loading" class="space-y-6">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p class="text-xs" style="color: var(--text-secondary)">Periksa dan verifikasi data hasil pembacaan
            ...
            <div class="keu-tab-group mb-4">
                <button @click="previewTab = 'stok'" class="keu-tab"
                    :class="previewTab === 'stok' ? 'keu-tab--active keu-tab--semua' : 'keu-tab--idle'">
                    Stok Masuk
                </button>
                <button @click="previewTab = 'penjualan'" class="keu-tab"
                    :class="previewTab === 'penjualan' ? 'keu-tab--active keu-tab--semua' : 'keu-tab--idle'">
                    Penjualan
                </button>
                <button @click="previewTab = 'supplier'" class="keu-tab"
                    :class="previewTab === 'supplier' ? 'keu-tab--active keu-tab--semua' : 'keu-tab--idle'">
                    Supplier
                </button>
                <button @click="previewTab = 'pembeli'" class="keu-tab"
                    :class="previewTab === 'pembeli' ? 'keu-tab--active keu-tab--semua' : 'keu-tab--idle'">
                    Pembeli
                </button>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody>
                        <template x-for="(item, idx) in previewData.stok" :key="idx">
                            <tr class="border-b hover:bg-gray-50/50 dark:hover:bg-slate-800/30"
                                ...
                                <td class="p-3 text-xs text-right font-medium" style="color: var(--text-primary)"
                                    x-text="formatQty(item.qty) + ' kg'"></td>
                                <td class="p-3 text-xs text-right font-medium" style="color: var(--text-primary)"
                                    x-text="formatQty(item.qty_actual) + ' kg'"></td>
                                <td class="p-3 text-xs text-right font-semibold" style="color: var(--text-primary)"
                    ...
                    <tbody>
                        <template x-for="(item, idx) in previewData.penjualan" :key="idx">
                            <tr class="border-b hover:bg-gray-50/50 dark:hover:bg-slate-800/30"
                                style="border-color: var(--border-color)">
                                <td class="p-3 text-xs" style="color: var(--text-primary)" x-text="item.tanggal"></td>
                                <td class="p-3 text-xs font-mono font-bold" style="color: var(--text-primary)"
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody>
                        <template x-for="(item, idx) in previewData.supplier" :key="idx">
                            <tr class="border-b hover:bg-gray-50/50 dark:hover:bg-slate-800/30"
                                style="border-color: var(--border-color)">
                                <td class="p-3 text-xs font-bold" style="color: var(--text-primary)" x-text="item.nama">
                                </td>
                                <td class="p-3 text-xs" style="color: var(--text-primary)"
                                    x-text="item.nama_pemilik || '-'"></td>
                                <td class="p-3 text-xs" style="color: var(--text-primary)" x-text="item.telpon || '-'">
                                </td>
                                <td class="p-3 text-xs" style="color: var(--text-primary)"
                    ...
                    <tbody>
                        <template x-for="(item, idx) in previewData.pembeli" :key="idx">
                            <tr class="border-b hover:bg-gray-50/50 dark:hover:bg-slate-800/30"
                                ...
                                <td class="p-3 text-xs font-semibold uppercase" style="color: var(--text-primary)"
                                    x-text="item.tipe"></td>
                                <td class="p-3 text-xs" style="color: var(--text-primary)" x-text="item.telpon || '-'">
                                </td>
                                <td class="p-3 text-xs" style="color: var(--text-primary)"
    ...
    <div x-show="tab === 'ocr'" class="space-y-6">
        ...
        <div x-show="!ocrPreviewMode && !ocrScanning"
            class="card p-8 text-center border-2 border-dashed border-gray-300 dark:border-slate-700 hover:border-amber-500 transition-colors cursor-pointer"
            ...
            <div class="flex flex-col items-center justify-center space-y-4">
                ...
                <div>
                    <button class="btn btn-outline border-amber-500/50 hover:bg-amber-500/10 px-6"
                        style="color: var(--text-primary)" @click="document.getElementById('ocrFile').click()">
                        Pilih Foto Catatan
                    </button>
                </div>
            </div>
        ...
        <div x-show="ocrScanning"
            class="card p-8 flex flex-col items-center justify-center space-y-6 relative overflow-hidden min-h-[300px]">
            ...
            <div class="text-center z-10 max-w-md">
                <h4 class="font-bold text-base" style="color: var(--text-primary)" x-text="ocrStatusTitle">Membaca
                    Tulisan Tangan...</h4>
                ...
                <p class="text-xs mt-2" style="color: var(--text-secondary)" x-text="ocrStatusDesc">Sonar OCR sedang
                    memproses citra...</p>
            </div>
        </div>
        ...
        <div x-show="ocrPreviewMode && !ocrScanning" class="space-y-6">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p class="text-xs" style="color: var(--text-secondary)">Perbaiki jika ada kesalahan baca sebelum
            ...
            <div class="card p-6 space-y-4">
                <h4 class="font-bold text-sm border-b pb-2 flex items-center gap-2"
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="border-b"
                                style="border-color: var(--border-color); background: rgba(0,0,0,0.02)">
                                ...
                                <tr class="border-b hover:bg-gray-50/50 dark:hover:bg-slate-800/30"
                                    ...
                                    <td class="p-2">
                                        <input type="date" x-model="item.tanggal"
                                            class="form-input text-xs w-full py-1 px-2">
                                    </td>
                                    <td class="p-2">
                                        <div class="flex flex-col gap-1">
                                            <input type="text" x-model="item.supplier"
                                                class="form-input text-xs w-full py-1 px-2 font-medium">
                                    ...
                                    <td class="p-2">
                                        <input type="number" step="any" x-model.number="item.qty"
                                            class="form-input text-xs w-full py-1 px-2 text-right font-medium"
                                    ...
                                    <td class="p-2">
                                        <input type="number" x-model.number="item.harga_beli"
                                            class="form-input text-xs w-full py-1 px-2 text-right font-semibold"
                                            @input="item.total = item.qty_actual * item.harga_beli">
                                    </td>
                                ...
                                <tr class="border-b hover:bg-gray-50/50 dark:hover:bg-slate-800/30"
                                    ...
                                    <td class="p-2">
                                        <input type="text" x-model="item.no_nota"
                                            class="form-input text-xs w-full py-1 px-2 font-mono font-bold">
                                    </td>
                                    <td class="p-2">
                                        <div class="flex flex-col gap-1">
                                    ...
                                    <td class="p-2">
                                        <input type="number" step="any" x-model.number="item.qty"
                                            class="form-input text-xs w-full py-1 px-2 text-right"
...
<?php $scripts = '<script src="/peace_seafood/inline-assets/js/master-data/migrasi.js"></script>'; ?>