...
<div x-data="keuanganPage()" x-init="init()">
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <button @click="showCreateModal = true; $nextTick(() => { if(window.lucide) lucide.createIcons(); })"
            class="btn btn-primary" x-show="['admin','bos'].includes(user.role)">
                ...
                <tbody>
                    ...
                    <template x-for="hp in filteredList" :key="hp.id">
                        <tr>
                            ...
                            <td>
                                <button x-show="hp.status !== 'lunas' && ['admin','bos'].includes(user.role)"
    ...
    <div class="modal-overlay" x-show="showBayarModal" @click.self="showBayarModal = false" x-cloak>
        <div class="modal-box">
            ...
            <div class="form-group">
                ...
                <input type="number" x-model="bayarForm.nominal_bayar" class="form-input" :max="selectedHp?.sisa_hutang"
                    min="1000" step="1000">
            </div>
            <div class="form-group">
            ...
            <div class="flex gap-3 mt-4">
                <button @click="openBayarConfirm()" class="btn btn-primary">Simpan Pembayaran</button>
    ...
    <div class="modal-overlay" x-show="showBayarConfirmModal" @click.self="showBayarConfirmModal = false" x-cloak>
        <div class="modal-box">
            <h3 class="font-bold text-lg mb-4">Konfirmasi Pembayaran</h3>
            <p class="text-sm mb-2" style="color: var(--text-secondary)">Pastikan nominal pembayaran sudah benar sebelum
                disimpan.</p>
            <p class="text-sm mb-4">
                <span style="color: var(--text-secondary)">Nominal:</span>
                <strong x-text="'Rp ' + parseFloat(bayarForm.nominal_bayar || 0).toLocaleString('id-ID')"></strong>
            </p>
            <div class="flex gap-3 mt-4">
                <button @click="submitBayar()" class="btn btn-primary">Ya, Simpan</button>
                <button @click="showBayarConfirmModal = false" class="btn btn-secondary">Batal</button>
            </div>
        </div>
    </div>
</div>
...
function keuanganPage() {
    return {
        ...
        async loadData() {
            ...
            try {
                ...
                const [listRes, agingRes] = await Promise.all([
                    axios.get('/peace_seafood/api/keuangan/hutang-piutang' + cacheBust, { headers }),
                    axios.get('/peace_seafood/api/laporan/keuangan' + cacheBust, { headers }),
                ]);
        ...
        async submitBayar() {
            this.showBayarConfirmModal = false;
            this.showBayarModal = false;
            if (!this.bayarForm.nominal_bayar) return;
            try {
                const token = localStorage.getItem('token');
                await axios.post('/peace_seafood/api/keuangan/bayar', this.bayarForm, { headers: { Authorization: 'Bearer ' + token } });
        ...
        openBayarConfirm() {
            if (!this.bayarForm.nominal_bayar) {
                iziToast.warning({ title: 'Perhatian', message: 'Nominal bayar wajib diisi.', position: 'topRight' });
                return;
            }

            this.showBayarConfirmModal = true;
        }
    };
}
</script>
